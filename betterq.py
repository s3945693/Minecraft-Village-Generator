    ...
    def worker_fn(mc_socket, request_iter, request_lock, answer_queue,):
        ...
        while more_requests or len(pending_request_queue) > 0:
            ...
            while more_requests and len(request_buffer) < 4096:
                with request_lock:
                    try:
                        ...
                        more_requests = False
                        continue
                    #print(type(fmt), type(request))
                    #print(fmt, request)
                    request_buffer = request_buffer + (fmt % request).encode() + b'\n' #removed + "\n" and replace it wiih encode
                    #print(type(request_buffer))
                    #print(request_buffer.decode())
            ...
            if allow_read:
                ...
                #print('here',type(response_buffer, bytes_read))
                response_buffer = response_buffer + bytes_read
                #print('yes 2')
                if bytes_read == 0:
                    raise RuntimeError("unexpected socket.recv()=0")
            ...
            #print(response_buffer.decode())
            #print(type(response_buffer))
            #print()
...
def try_multiple_threads_socket_stuffing(input_cuboid):
    ...
    #  25       12.7            5160
    #  30       13.2            4964
def getMap(start,end, doorCoords,doorY):
    
    myCord = [] #a sorted list of cords
    myBlocks = [] #created a tuple of block type and its coord
    tempCord = [] #a list of cords that are not sorted
    tempMap = [] #a list of blocks that are not sorted to be used to make a map
    sortedMyBlocks = [] #a sorted list of blocks
    tempSortedMyBlocks = [] #a list of blocks that are not sorted
    x = start[0]
    y = start[1]
    z = start[2]
    ...
    #xInterval = int(end[0])-x+30 #x axis
    prevCoord = x-15 #used for sorting myCord by x values later on
    forGetBlock = [] #only used for getHeight function
    cube = Cuboid((x-15, x+150), (0, 1), (z-105, z+105)) #creates a cube to parse into the quick search function, increase values if index out of range error occurs
    print('quering height')
    
    for pos, yA in query_blocks(cube.generate_xz(), "world.getHeight(%d,%d)", int, thread_count=30): #gets the height of the block area   
        
        if pos[0] == end[0] and pos[1] == end[1]: #if the current block is the end block, doesn't get store highest block
            forGetBlock.append(end)
        elif pos[0] == x and pos[1] == z:  #if the current block is the start block, doesn't get store highest block
            forGetBlock.append((x,y,z))
        elif (pos[0],pos[1]) in doorCoords:
            thisIsValue = doorCoords.index((pos[0],pos[1]))
            forGetBlock.append((pos[0],doorY[thisIsValue],pos[1]))
        else:
            if(pos[0],yA,pos[1]) not in forGetBlock: #if the current block is not the start or end block, store the highest block
                forGetBlock.append((pos[0], yA, pos[1])) #used to parse into next function
    
    #forGetBlock = tuple(forGetBlock) #converts the list to a tuple
    myBlocks = forGetBlock
    myBlocks.sort(key= operator.itemgetter(0,2)) #sorts the list by x and z value
    print('got height and data sorted')
    maps = [] #use to create a map to parse into aStar
    
    print('creating map for aStar')
    for i, x in enumerate(myBlocks): #creates a map of the block area and also creates a sorted list of the coordinates

        

        if prevCoord < x[0]: #creates the sorted list of coordinates
            myCord.append(tempCord)
            maps.append(tempMap)
            tempSortedMyBlocks = []
            tempCord = []
            tempMap = []
        ...
        tempStore = int(x[1]) 
        tempMap.append(tempStore)
            
        tempCord.append((x[0], x[1], x[2]))
